...
│   ├── WorklogGhost.tsx      # фоновая анимация внутри карточек Worklog (отскакивающее привидение)
│   ├── ChainLink3D.tsx          # декоративный 3D-элемент (Three.js)
│   └── ParallaxBackdrop.tsx      # фоновые привидения: параллакс, парение, клик-пасхалка
└── app/page.tsx           # главная страница, связывает всё вместе
```

Визуальный стиль оформлен под референс mox-studio.ru (палитра, шрифт Montserrat как
открытый аналог их Monts, wordmark-заголовок) — это уже сверх минимума из ТЗ, на
логику подбора и обязательные состояния не влияет.
